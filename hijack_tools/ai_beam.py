...
try:
    from algo_config import (
        ...
        DANGER_BASE, SAFETY_MARGIN,
        WALL_PENALTY, WALL_MARGIN,
        TIME_WEIGHT_BASE, TIME_WEIGHT_RATE,
        EARLY_EXIT_ENABLED, EARLY_EXIT_BUFFER,
        PARTIAL_SORT_ENABLED,
...
class BeamAI:
    ...
    def _config_kwargs(self):
        return dict(
            beam_width=BEAM_WIDTH, beam_depth=BEAM_DEPTH, check_every=CHECK_EVERY,
            danger_base=DANGER_BASE, safety_margin=SAFETY_MARGIN,
            wall_penalty=WALL_PENALTY, wall_margin=WALL_MARGIN,
            tw_base=TIME_WEIGHT_BASE, tw_rate=TIME_WEIGHT_RATE,
            early_exit_enabled=EARLY_EXIT_ENABLED, early_exit_buffer=EARLY_EXIT_BUFFER,
            partial_sort_enabled=PARTIAL_SORT_ENABLED,
            center_pull_enabled=CENTER_PULL_ENABLED, wall_penalty_enabled=WALL_PENALTY_ENABLED,
        )
    ...
    def decide(self, px, py, bullets):
        ...
        best = int(_beam_search(float(px), float(py), paths, **self._config_kwargs()))
